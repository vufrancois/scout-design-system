...
- **Cancellation resolves through the vendor, and Canceled is a terminal state with no stepper:** a buyer's request (Cancellation Pending, sky on the buyer side) arrives as the vendor's rose *Cancellation Requested* pill and a rose-tinted "Action Required · Cancel Order" strip (`#cancelreq`). The Cancel modal states the consequences (items to stock, shipping waived, stale invoice removed, buyer notified; history stays visible) and demands a **required cancellation reason, visible to the buyer** (500-char cap with counter) — Cancel stays disabled without it. Confirming lands on `#canceled`: the Order Progress stepper is *removed entirely* (a pipeline that will never complete is not shown as one), the header pill reads rose Canceled, and the strip becomes the terminal record quoting the reason. The buyer's canceled order (`buyer/order-detail.html#153`) mirrors it: no stepper, no task strip, no claim entries, no fulfillment cards — an **Order Status card** carries the Canceled pill, "Canceled by *vendor* · date", the quoted reason, and "Nothing was charged — order history remains visible." The same Cancel Order flow also covers the all-merchandise-canceled case (every line claim-canceled — cancel to waive shipping and close the balance). **Declining is symmetric:** the strip's secondary "Decline Request" opens its own modal demanding a *required, buyer-visible reason* ("the buyer keeps their claim rights — declining only ends the cancellation request"); confirming returns the order to its normal path with "Cancellation declined" in both audit trails, and the buyer sees the quoted reason in a rail notice while the lifecycle simply continues (`buyer/order-detail.html#420`).
...
- The Buyer App uses a **floating pill App Bar** (card surface, pill radius, shadow-md, sticky under a 14px gutter) instead of the vendor sidebar. **Exception:** on the product listing page the bar is static and scrolls away — while browsing results, the persistent chrome is the in-page search/sort bar and filter rail (re-anchored to the top), not the nav; three stacked sticky layers read as clutter. Anatomy left→right: avatar ring (36px, 2px primary border, initials) + Scout wordmark (inline SVG on `currentColor`), Nav Chips, Context Switcher, icon buttons.
- **Nav Chips:** 38px pills, 14px/500, 17px muted leading icon; active = muted fill + 600 weight + foreground icon; hover = accent. Dropdown chips carry a chevron that rotates 180° while open. Under 1280px labels collapse to icons (raised from 1100px when the search icon joined the bar); under 820px the wordmark hides.
- **Icon buttons:** 40px circles; primary-filled only for notifications and cart (the two the reference filled); a status dot (emerald, 2px surface ring) marks unread. A **search icon** leads the cluster on every page — product search costs the bar no width.
- **Search Palette:** the search icon (or ⌘K / / anywhere) opens a centered command-palette overlay: one large input with the Keyword / AI Search mode segment carried over from the marketplace hero, recent searches, category jump rows, and a footer of keyboard hints plus the **property scope** ("Scoped to *property*" — searches honor the Shopping-for choice). This is the app-level search: it gives every non-shopping page reach into the catalog without adding a field to the bar; the shopping pages keep their own in-page search surfaces. Esc or clicking outside closes; injected once from the shared script, never duplicated per page.
...
- **Shopping surfaces require one property — "All" is not a shopping context.** Products, pricing, and availability are tailored per property, so on the marketplace surfaces (marketplace, listings, product detail, compare) the switcher drops the "All properties" option, its kicker reads *"Shopping for"*, and an unset visitor is met by the **Property Gate**: a centered modal ("Choose a property to shop for · Products, pricing, and availability are tailored to each property") listing the property options — no dismiss affordance; picking is the only way through. The choice persists (`scout-shop-prop`) across all shopping pages and pre-answers the cart's property, and the switcher remains available to change it anytime. Aggregation pages (Home, Orders, Approvals, Wishlists) keep "All properties" — scope aggregation is legitimate where nothing is being bought.
...
- **Fixed-slot anatomy** so every card in a row aligns: media (4:3 — pickup badge bottom-left, compare toggle top-right, and the **vendor avatar bottom-right**: a 34px card-surface circle with initials mirroring the compare toggle, the vendor name surfacing as a hover tooltip anchored above-right to stay inside the frame) → title (15px/600, **two lines always reserved**) → price block of **two fixed lines** (24px price with `/ each` inline and unwrappable; compare-at strikethrough · "Save 26%" as emerald text beneath, reserved even when empty) → two single-line meta rows sharing a 14px icon column (delivery promise, stock) → Add to Cart + wishlist pinned to the bottom.
- Vocabulary: units read "/ each" (never EA / EACH); delivery promises are complete sentences, never truncated chips ("Overnight · order by 2pm CST", "3–5 business days", "5–7 business days · from vendor"); stock is In Stock / Low stock / Out of stock (out disables Add to Cart); vendors show an avatar tile with the name on hover, not an inline logo.
...
- Buy Box order: 22px title → fact rows (mono SKU, linked Category, Brand) → Delivery Options → Quantity Stepper → price block → action row → vendor panel → ratings. **Exactly one primary** (Add to cart); Buy now and Chat with seller are outline; Explore AI alternatives carries the `.btn-ai` accent (sky-soft fill, sky border and text, sparkle icon) — AI features get the sky accent, obvious at a glance without competing with the navy primary; wishlist and Compare are icon buttons.
...
- Product details are a full-width card below both columns with underline tabs (AI Summary · Description · Specifications · Spec Sheets · Reviews); content fills the card (two-column bullets and spec grid when wide). The Reviews tab is the buy box's "See reviews" destination (activates the tab and scrolls to it); with no reviews it shows an honest empty state — five hollow stars, "Reviews come from verified property managers after delivery is validated," and the Write a Review entry. Alternatives render as a sortable **grid**, not a carousel — a sort control on a rail reorders items you can't see.
...
- **Decision Panel:** pending requests only, directly under Approval Progress, amber left accent. One comment field serves all three actions; the requirement is stated up front — the field label "Comment to the buyer" carries an amber *Required for revision or rejection* chip, and Request revision / Reject start disabled until the comment has text (the inline hint flips from "Approving needs no comment." to "Your comment goes to the buyer with the decision."). The rose field ring remains as a fallback. Exactly one primary (Approve); Request revision is a sky outline, Reject a rose outline. Deciding transitions the page to the resulting state and appends the history event ("Decided by You").
...
- **One request per checkout, any number of vendors:** an approval request covers the whole over-budget checkout, not one vendor's slice — approving is one decision, and it creates **one order per vendor** (the "Orders placed" rail card lists each order with its vendor and amount). In the table, a multi-vendor request shows stacked vendor avatars with an "N vendors" label (full names in the tooltip), the vendor filter and search match any vendor on the request, and the expanded Items sub-row groups lines under vendor sub-headers. On the detail page, Order details groups items per vendor (avatar + name + item count · vendor subtotal), the rail's vendor card stacks one entry per vendor ("Vendor 1 of N"), and totals still close as a single Request total — one request, one number, N orders out.
...
- **The row highlight is reserved for claims:** a row with an open claim takes a rose-tinted fill and a rose warning triangle beside the order number (tooltip: "Claim open — the invoice is on hold until it's resolved") — the gravest table state gets the loudest treatment, on any tab. Routine your-move rows (delivered awaiting validation, invoice received) carry no tint or icon; the navy **Validate** primary is their cue. One highlight, one meaning.
