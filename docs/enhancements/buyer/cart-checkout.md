...
- Review refinements: per-card Edit link back to #shipping (and Buyer details carries one back to #address); item disclosures auto-open on entering review (last look at the goods); rail vendor blocks slimmed to name + check + total (dropping the duplicated items/method/payment lines); "(1 items)" pluralization fixed across cart + checkout rails. Kept: radio marks on selected rows, and the total on the place button (self-sufficient commitment).
